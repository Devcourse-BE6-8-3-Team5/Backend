...
class FactQuizService(
    ...
    fun findByRank(rank: Int): List<FactQuizDto> {
        val nthRankNews = realNewsRepository.findNthRankByAllCategories(rank)
    ...
    fun findByCategoryAndRank(category: NewsCategory, rank: Int): Optional<FactQuizDto> {
        val realNews = realNewsRepository.findNthRankByCategory(category, rank)

        if (realNews.isEmpty) {
            return Optional.empty()
        }

        return factQuizRepository.findByRealNewsId(realNews.get().id)
    ...
    fun findById(id: Long, actor: Member): FactQuizWithHistoryDto {
        ...
        val quizHistory = histories.firstOrNull { it.quizId == id && it.quizType == QuizType.FACT }

        // 퀴즈 히스토리가 없으면 null 반환
        return if (quizHistory == null) {
            FactQuizWithHistoryDto(factQuizDto, null, false, 0)
        } else {
            FactQuizWithHistoryDto(factQuizDto, quizHistory.answer, quizHistory.isCorrect, quizHistory.gainExp)
        }
    }
    ...
    fun submitDetailQuizAnswer(actor: Member, id: Long, selectedNewsType: CorrectNewsType): FactQuizAnswerDto {
        ...
        managedActor.exp += gainExp

        managedActor.level = calculateLevel(managedActor.exp) // 레벨 계산 로직 추가
